...
model <- lm(price ~ house_size + bath + bed + state + status, data = train_data)
tidy(model) |>
  arrange(p.value) |>
  mutate(
...
Die Grid-Suche zeigt, wie sich die Modellgüte (RMSE) in Abhängigkeit von num.trees und mtry verändert:

num.trees
Mehr Bäume reduzieren die Varianz des Modells und stabilisieren die Vorhersagen.
Ab einer bestimmten Anzahl (hier typischerweise ≥ 300) verbessert sich der RMSE jedoch nur noch marginal.

mtry
Steuert die Anzahl der Variablen, die bei jedem Split betrachtet werden.
Kleinere Werte erhöhen die Zufälligkeit und reduzieren Overfitting, zu kleine Werte können jedoch relevante Prädiktoren verdrängen.

Die beste Kombination (niedrigster RMSE) wird im finalen Random-Forest-Modell verwendet.
...
size (Hidden Neurons)
Bestimmt die Modellkomplexität.
Zu kleine Netze unterfitten, zu große Netze neigen zu Overfitting.

decay (Weight Decay / Regularisierung)
Bestraft große Gewichte und stabilisiert das Training.
Moderate Werte (z. B. 0.01) führen häufig zu besserer Generalisierung.

Die Kombination mit dem niedrigsten RMSE stellt den besten Kompromiss zwischen Modellkomplexität und Generalisierungsfähigkeit dar.
## Neural Network
